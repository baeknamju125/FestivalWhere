...
import kr.hs.emirim.festivalwhere.HLog;
...
public class OneMonthView extends LinearLayout implements View.OnClickListener {
    ...
    public void make(int year, int month)
    {
        ...
        festivals.put("2016-09-21",true);
        festivals.put("2016-08-27",true);
        festivals.put("2016-07-27",true);
        festivals.put("2016-09-27",true);

        OneDayData tempOne ;
        String tempDate;
        for(int i = 0; i < oneDayDataList.size(); i++){
            tempOne = oneDayDataList.get(i);
            tempDate = tempOne.get(Calendar.YEAR) + "-";
            if((tempOne.get(Calendar.MONTH) + 1) < 10) tempDate += "0" + (tempOne.get(Calendar.MONTH) + 1);
            else tempDate += "" + (tempOne.get(Calendar.MONTH) + 1);
            if(tempOne.get(Calendar.DAY_OF_MONTH) < 10) tempDate += "-0" + tempOne.get(Calendar.DAY_OF_MONTH);
            else tempDate += "-" + tempOne.get(Calendar.DAY_OF_MONTH);

            if(festivals.containsKey(tempDate)) {
                tempOne.isFestivalDay = true;
            }
        }
